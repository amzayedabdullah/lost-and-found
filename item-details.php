...
<div class="resp-grid-sidebar">
    ...
    <div>
        ...
        <div class="dash-stat-box" style="padding: 30px; margin-bottom: 20px;">
            ...
            <p style="color: var(--dash-text-muted); font-size: 0.9rem; margin-bottom: 20px;"><?php echo __('your_item_desc'); ?></p>
            <form method="post" action="item-details.php?id=<?php echo $item['id']; ?>">
                <?php if ($item['status'] != 'resolved'): ?>
                    <input type="hidden" name="owner_action" value="mark_resolved">
                    <button type="submit" class="btn btn-primary" style="width: 100%; padding: 14px; border-radius: 14px; font-size: 1rem; display: flex; align-items: center; justify-content: center; gap: 10px; background: #10b981;">
                        <i class="fas fa-check-circle"></i> Mark as Resolved
                    </button>
                <?php else: ?>
                    <input type="hidden" name="owner_action" value="reopen_item">
                    <button type="submit" class="btn btn-outline" style="width: 100%; padding: 14px; border-radius: 14px; font-size: 1rem; display: flex; align-items: center; justify-content: center; gap: 10px; border-color: #3b82f6; color: #3b82f6; background: white;">
                        <i class="fas fa-redo"></i> Post Item Again
                    </button>
                <?php endif; ?>
            </form>
        </div>

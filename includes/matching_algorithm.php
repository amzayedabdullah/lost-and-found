...
function getPotentialMatches($pdo, $itemId, $keywords, $type) {
    ...
    $src_keywords_arr = $extract_words($src_item['keywords']);
    ...
    // The twist requires "highlighting items with at least 2 matching keywords"
    $sql = "SELECT * FROM items WHERE type = :type AND status = 'open' AND id != :id AND (keywords REGEXP :pattern OR title REGEXP :pattern)";
    ...
    while($row = $stmt->fetch()) {
        ...
        // Requirement: Must have at least 2 matching keywords
        if ($keyword_matches_count < 2) {
            continue;
        }
        ...
        if($match_percentage >= 50) {
            ...
            // Background Task: If this is an extremely strong match (Image + Keywords >= 2), 
            // trigger Admin Verification by inserting into system_matches
            if ($has_image_match && $keyword_matches_count >= 2) {

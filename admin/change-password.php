...
<div style="max-width: 600px;">
    ...
    <div class="dash-stat-box">
        <div style="display: flex; align-items: center; gap: 20px; margin-bottom: 35px;">
            ...
            <div>
                <h3 style="font-weight: 700; margin-bottom: 4px;"><?php echo __('admin_security'); ?></h3>
                <p style="color: var(--dash-text-muted); font-size: 0.9rem;"><?php echo __('admin_security_desc'); ?></p>
            </div>
        </div>
        ...
        <form action="change-password.php" method="post">
            <div class="form-group">
                ...
                <div style="position: relative; margin-top: 8px;">
                    <input type="password" name="current_password" id="current_pw" class="form-control" placeholder="Enter your current password" style="border-radius: 14px; padding: 14px 50px 14px 18px;" required>
            ...
            <div class="form-group">
                ...
                <div style="position: relative; margin-top: 8px;">
                    <input type="password" name="new_password" id="new_pw" class="form-control" placeholder="Minimum 8 characters" style="border-radius: 14px; padding: 14px 50px 14px 18px;" required minlength="8">
            ...
            <div class="form-group">
                ...
                <div style="position: relative; margin-top: 8px;">
                    <input type="password" name="confirm_password" id="confirm_pw" class="form-control" placeholder="Re-enter new password" style="border-radius: 14px; padding: 14px 50px 14px 18px;" required minlength="8">
            ...
            <div style="display: flex; gap: 15px; margin-top: 35px;">
                <button type="submit" class="btn btn-primary" style="padding: 14px 35px; border-radius: 14px; font-size: 1rem; background: #ef4444;">
                    <i class="fas fa-lock"></i> Update Password
                </button>
                <a href="dashboard.php" class="btn btn-outline" style="padding: 14px 35px; border-radius: 14px; text-decoration: none;"><?php echo __('cancel'); ?></a>
            </div>
        </form>
    </div>
</div>
...
document.getElementById('new_pw').addEventListener('input', function() {
    ...
    for (let i=1;i<=4;i++) document.getElementById('str'+i).style.background = i<=score?colors[score-1]:'#e2e8f0';
...
document.getElementById('confirm_pw').addEventListener('input', function() {
    const m = this.value === document.getElementById('new_pw').value, el = document.getElementById('match-text');
    if (this.value.length>0) { el.textContent = m?'✓ Passwords match':'✗ Passwords do not match'; el.style.color = m?'#10b981':'#ef4444'; }
    else el.textContent = '';
});
</script>
